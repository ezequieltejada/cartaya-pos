...
```
{
  data: Array<{ id: string, name: string, sku: string | null, description: string | null, category: { categoryId: string, name: string } | null, active: boolean, defaultPriceId: string | null }>,
  pagination: { total: number, limit: number, offset: number, hasMore: boolean }
}
```
...
Categories are tenant-scoped, stored normalized (trimmed, collapsed whitespace, lowercase), and ordered by integer `position`.
...
**Notes:**
- Name is normalized and stored in lowercase.
- New categories are appended to the end (`position = max + 1`).
...
```
{
  data: Array<{ id: string, name: string, position: number, createdAt: string, updatedAt: string }>,
  pagination: { total: number, limit: number, offset: number, hasMore: boolean }
}
```
...
### GET /api/tenants/:tenantId/categories/:categoryId
...
### PATCH /api/tenants/:tenantId/categories/:categoryId
...
**Response (200):** (same as GET category)
...
### PATCH /api/tenants/:tenantId/categories/reorder
...
These endpoints are used to serve public assets like product images. They do not require authentication.

### GET /media/pictures/:tenantId/:productId/:filename

Serves a public image file.

**Purpose:** Allows the frontend to display product images using standard `<img>` tags. Images are publicly accessible to ensure fast loading and compatibility with CDNs.

**Parameters:**
- `tenantId` (string, required): The tenant identifier
- `productId` (string, required): The product identifier
- `filename` (string, required): The image filename (e.g., `image.jpg`)

**Response (200):**
- Binary image data with appropriate `Content-Type` header (e.g., `image/jpeg`)
- `Cache-Control: public, max-age=31536000`

**Supported Formats:**
The server supports the following image extensions:
- `.jpg` / `.jpeg`
- `.png`
- `.gif`
- `.webp`
...
**Error Responses:**
- `400`: Invalid tenantId, productId, filename, or unsupported extension
